...
class OeHealthPatientEvaluation(models.Model):
    ...
    doctor = fields.Many2one('oeh.medical.physician', string='Doctor', help="Current primary care / family doctor", domain=[('is_pharmacist','=',False)], required=True, default=_get_physician)
    ...
    derived_from = fields.Many2one('oeh.medical.physician', string='Doctor who escalated the case')
    derived_to = fields.Many2one('oeh.medical.physician', string='Doctor to whom escalated')
    evaluation_type = fields.Selection(EVALUATION_TYPE, string='Evaluation Type', required=True, index=True, default=lambda *a: 'Pre-arranged Appointment')
    ...
    def create(self, vals):
        company = self.env['res.company']._company_default_get('oeh.medical.evaluation')
        search_sequence = self.env['ir.sequence'].search(
            [('code', '=', 'oeh.medical.evaluation'), ('company_id', 'in', [company.id, False])], order='company_id')
        if not search_sequence:
            values = {
                'name': 'Evaluation (' + str(company.name) + ')',
                'code': 'oeh.medical.evaluation',
                'company_id': company.id,
                'prefix': 'EN',
                'padding': 6,
            }
            self.env['ir.sequence'].sudo().create(values)
            vals['name'] = self.env['ir.sequence'].next_by_code('oeh.medical.evaluation')
            medical_evaluation = models.Model.create(self, vals)
