...
class oeHealthRESTAPIController(http.Controller, SmartMindSharedMethods):
    ...
    def get_service_list(self, **payload):  # , type=None
        ...
        model = request.env[self._model].search([('model', '=', model_name)], limit=1)
        payload[
    ...
    def get_doctor_schedule(self, doctor_id=None, **payload):
        ...
        model = request.env[self._model].sudo().search([('model', '=', model_name)], limit=1)
        payload['fields'] = "['id', 'name', 'date', 'duration', 'start_time', 'end_time']"

        today = datetime.strptime(self._convert_utc_to_local(str(datetime.now()).split('.')[0]), '%Y-%m-%d %H:%M:%S')
        ...
        if model:
            ...
            if doctor_id:
                domain = [('physician_id.id', '=', str(doctor_id)), ('date', '>', today_date)]
            else:
            ...
            for x in data:
                ...
                for k, v in x.items():
                    ...
                    if k == 'duration':
                        duration = v
                # build times depends on start and end time from db and increment times from duration
                # get hours, we need this to use it for limit max loop value
                hours = int(end_time - start_time)
                # convert time float to time string, example: 08:00
                str_time = '{0:02.0f}:{1:02.0f}'.format(*divmod(float(start_time) * 60, 60))
                given_time = datetime.strptime(str_time, '%H:%M')
                # convert above time string to time, example: 1900-01-01 08:00:00
                lst = []  # we need this list to store times, examples: ['08:30', '09:00', '09:30', '10:00', '10:30', '11:00', '11:30', '12:00']
                # if sch_date >= today_date:
                # start increment with duration from db
                counter = 0
                # loop from start time (08:00) to last time (12:00), counter start from duration
                # and add duration to counter each process to get list correct times
                if sch_date > today_date:
                    if duration > 0:
                        while counter < (60 * hours):
                            ...
                            if self._is_appointment_date_found(sch_date, spilt_time):
                                lst.remove(spilt_time)
                            counter += duration
                    else:
                        return invalid_response('invalid duration', 'You cannot make a duration of any schedule = 0')

                # create new key in json output named list with values of list
                x['list'] = lst
                data_to_push.append(x)
            return valid_response(data_to_push)
        return invalid_response('invalid object model', 'The model %s is not available in the registry.' % model_name)
    ...
    @authenticate_token
    @http.route(['/sehati/get-doctor-schedule-n/<doctor_id>'], type='http', auth="none", methods=['GET'], csrf=False)
    def get_doctor_schedule_n(self, doctor_id=None, **payload):
        # check if doc has schedu in sch tab
        sch_model = 'sm.shifa.physician.schedule'
        sch_domain = [('id', '=', int(doctor_id))]
        count_sch = request.env[sch_model].sudo().search_count(sch_domain)
        if count_sch > 0:
            schedul = request.env[sch_model].sudo().search(sch_domain, limit=1)
            # get id of sch for that doc and use in domaon of shc lines
            ...
            if model:
                ...
                domain = [('schedule_id.id', '=', str(schedul.id)), ('date', '>', today_date)]
                data = request.env[model.model].sudo().search_read(
                    domain=domain, fields=fields, offset=offset, limit=limit, order=order)
                ...
                for x in data:
                    ...
                    for k, v in x.items():
                        if str(type(v)) in ("<class 'datetime.datetime'>", "<class 'datetime.date'>"):
                            ...
                            sch_date = date
                            if sch_date > today_date:
                                lst = self.get_timeslote_list(doctor_id, sch_date)
                        if k == 'start_time':
                            start_time = v
                        if k == 'end_time':
                            end_time = v
            ...
            return invalid_response('schedul_not_fouond', 'The given doctor id has no schdule, please select another doctor')
        ...
        if model:
            try:
                ...
                data = {
                    ...
                    'tele_appointment': str(record.tele_appointment.name),
                    'date_tele_appointment': str(record.date_tele_appointment),
    ...
    def get_requested_payments(self, patient_id=None, **payload):
        ...
        model = request.env[self._model].sudo().search([('model', '=', model_name)], limit=1)
        payload[
            'fields'] = "['id', 'name', 'state', 'payment_amount', 'payment_reference', 'date', 'details', 'type', 'hhc_appointment', 'tele_appointment', 'hvd_appointment', 'phy_appointment', 'pcr_appointment']"
        if model:
            domain, fields, offset, limit, order = extract_arguments(payload)
    ...
    def calculate_schedule_availability_time(self, start_time, end_time, duration):
        ...
        while duration <= (60 * hours):
            ...
            list.append(spilt.strftime('%H:%M'))
            counter += duration
        return list
    ...
    def get_timeslote_list(self, doctor_id, date, **payload):
        ...
        if model:
            domain, fields, offset, limit, order = extract_arguments(payload)
            domain = [('physician_id', '=', str(doctor_id)), ('date', '=', date)]
