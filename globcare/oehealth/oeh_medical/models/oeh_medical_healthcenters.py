...
class OeHealthCentersBuilding(models.Model):
    ...
    _sql_constraints = [
        ('name_uniq', 'unique (company_id,name)', 'The building name must be unique !')
    ]
...
class OeHealthCentersWards(models.Model):
    ...
    _sql_constraints = [
        ('name_ward_uniq', 'unique (company_id,name,building)', 'The ward name is already configured in selected building !')
    ]
