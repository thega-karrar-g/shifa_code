...
class SmMainPatientMedicine(models.Model):
    ...
    dose = fields.Float(string="Dose", related="medication_profile_id.p_dose", store=True)
    dose_unit = fields.Many2one('oeh.medical.dose.unit', related="medication_profile_id.p_dose_unit",
                                string="Dose Unit", store=True)
    ...
    dose_route = fields.Many2one('oeh.medical.drug.route', related="medication_profile_id.p_dose_route", string='Administration Route', store=True)
    ...
    def get_medicine_details(self):
        if self.prescribed_medicine:
            self.medicine = self.prescribed_medicine.name
            patient_profile = self.patient.med_pro_id
            profile_obj = self.env['sm.shifa.medication.profile'].search([('patient', '=', self.patient.id),
                                                                          ('state_app', '=', 'active'),
                                                                          ('p_generic_name', '=', self.prescribed_medicine.id)],limit=1)
            self.prescribed_dose = profile_obj.p_dose
            self.prescribed_dose_unit = profile_obj.p_dose_unit.id
            self.prescribed_dose_route = profile_obj.p_dose_route.id
            for i in patient_profile:
                if self.prescribed_medicine == i.p_generic_name:
                    #print(i)
                    self.medication_profile_id = i.id
...
class MedicineSchedule(models.Model):
    ...
    patient = fields.Many2one('oeh.medical.patient', related='caregiver_id.patient')
    patient_medicine_id = fields.Many2one('sm.caregiver.main.patient.medicine', string="Medicine Ref")
    caregiver = fields.Many2one('oeh.medical.physician', string='Caregiver', domain=[('role_type', '=', 'HD'), ('active', '=', True)],)
