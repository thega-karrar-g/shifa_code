...
class OeHealthSurgeryRCRI(models.Model):
    ...
    doctor = fields.Many2one('oeh.medical.physician', string='Doctor', domain=[('is_pharmacist','=',False)], help="Health professional / Cardiologist who signed the assesment RCRI")
...
class OeHealthSurgery(models.Model):
    ...
    def create(self, vals):
        ...
        company = self.env['res.company']._company_default_get('oeh.medical.surgery')
        ...
        if not search_sequence:
            values = {
                'name': 'Surgeries (' + str(company.name) + ')',
                'code': 'oeh.medical.surgery',
                'company_id': company.id,
                'prefix': 'SR/(' + datetime.datetime.now().strftime('%Y') + ')/',
                'padding': 5,
            }
            self.env['ir.sequence'].sudo().create(values)
            vals['name'] = self.env['ir.sequence'].next_by_code('oeh.medical.surgery')
