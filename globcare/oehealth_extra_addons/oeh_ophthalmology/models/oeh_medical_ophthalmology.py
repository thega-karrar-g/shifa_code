...
class OeHealthOphthalmology(models.Model):
    ...
    doctor = fields.Many2one('oeh.medical.physician','Doctor', domain=[('is_pharmacist','=',False)], help="Health professional / Ophthalmologist / OptoMetrist", required=True, readonly=True, states={'Draft': [('readonly', False)]}, default=_get_physician)
    ...
    def create(self, vals):
        company = self.env['res.company']._company_default_get('oeh.medical.ophthalmology')
        search_sequence = self.env['ir.sequence'].search(
            [('code', '=', 'oeh.medical.ophthalmology'), ('company_id', 'in', [company.id, False])],
            order='company_id')
        if not search_sequence:
            values = {
                'name': 'Ophthalmology (' + str(company.name) + ')',
                'code': 'oeh.medical.ophthalmology',
                'company_id': company.id,
                'prefix': 'OP',
                'padding': 5,
            }
            self.env['ir.sequence'].sudo().create(values)
            vals['name'] = self.env['ir.sequence'].next_by_code('oeh.medical.ophthalmology')
            medical_ophthalmology = models.Model.create(self, vals)
