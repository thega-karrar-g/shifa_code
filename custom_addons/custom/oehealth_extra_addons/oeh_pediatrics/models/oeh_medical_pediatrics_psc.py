...
class OeHealthPediatricSymptomsChecklist(models.Model):
    ...
    doctor = fields.Many2one('oeh.medical.physician', string='Doctor', domain=[('is_pharmacist','=',False)], help="Current primary care / family doctor", required=True)
    ...
    def create(self, vals):
        company = self.env['res.company']._company_default_get('oeh.medical.pediatrics.psc')
        search_sequence = self.env['ir.sequence'].search(
            [('code', '=', 'oeh.medical.pediatrics.psc'), ('company_id', 'in', [company.id, False])],
            order='company_id')
        if not search_sequence:
            values = {
                'name': 'Pediatric Symptom Checklists (' + str(company.name) + ')',
                'code': 'oeh.medical.pediatrics.psc',
                'company_id': company.id,
                'prefix': 'PCS',
                'padding': 5,
            }
            self.env['ir.sequence'].sudo().create(values)
            vals['name'] = self.env['ir.sequence'].next_by_code('oeh.medical.pediatrics.psc')
            medical_pediatrics_psc = models.Model.create(self, vals)
            return medical_pediatrics_psc
        else:
