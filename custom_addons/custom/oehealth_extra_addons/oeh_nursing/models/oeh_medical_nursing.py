...
class OeHealthPatientRoundingManagement(models.Model):
    ...
    doctor = fields.Many2one('oeh.medical.physician', string='Doctor', help="Doctor Name", domain=[('is_pharmacist','=',False)], required=True, readonly=True,states={'Draft': [('readonly', False)]}, default=_get_patient_rounding)
    ...
    def create(self, vals):
        company = self.env['res.company']._company_default_get('oeh.medical.patient.rounding')
        search_sequence = self.env['ir.sequence'].search(
            [('code', '=', 'oeh.medical.patient.rounding'), ('company_id', 'in', [company.id, False])],
            order='company_id')
        if not search_sequence:
            values = {
                'name': 'Rounding(' + str(company.name) + ')',
                'code': 'oeh.medical.patient.rounding',
                'company_id': company.id,
                'prefix': 'ROUND ' + datetime.datetime.now().strftime('%Y') + '/',
                'padding': 5,
            }
            self.env['ir.sequence'].sudo().create(values)
            vals['name'] = self.env['ir.sequence'].next_by_code('oeh.medical.patient.rounding')
            medical_patient_rounding = models.Model.create(self, vals)
            return medical_patient_rounding
        else:
...
class OeHealthPatientAmbulatoryCare(models.Model):
    ...
    patient = fields.Many2one('oeh.medical.patient', string='Patient', help="Patient Name", required=True, readonly=True, states={'Draft': [('readonly', False)]})
    doctor = fields.Many2one('oeh.medical.physician', string='Doctor', help="Doctor Name", domain=[('is_pharmacist','=',False)], required=True, readonly=True,states={'Draft': [('readonly', False)]}, default=_get_patient_ambulatory)
    ordering_doctor = fields.Many2one('oeh.medical.physician', string='Requested by', help="Doctor Name", domain=[('is_pharmacist','=',False)], readonly=True,states={'Draft': [('readonly', False)]})
    evaluation_start_date = fields.Datetime(string='Start date & time', required=True, readonly=True, states={'Draft': [('readonly', False)]}, default= lambda *a: datetime.datetime.now())
    evaluation_end_date = fields.Datetime(string='End date & time', readonly=True, states={'Draft': [('readonly', False)]})
    systolic = fields.Integer(string='Systolic Pressure', readonly=True, states={'Draft': [('readonly', False)]})
    diastolic = fields.Integer(string='Diastolic Pressure', readonly=True, states={'Draft': [('readonly', False)]})
    ...
    @api.model
    def create(self, vals):
        company = self.env['res.company']._company_default_get('oeh.medical.patient.ambulatory')
        search_sequence = self.env['ir.sequence'].search(
            [('code', '=', 'oeh.medical.patient.ambulatory'), ('company_id', 'in', [company.id, False])],
            order='company_id')
        if not search_sequence:
            values = {
                'name': 'Ambulatory Cares (' + str(company.name) + ')',
                'code': 'oeh.medical.patient.ambulatory',
                'company_id': company.id,
                'prefix': 'AC ' + datetime.datetime.now().strftime('%Y') + '/',
                'padding': 6,
            }
            self.env['ir.sequence'].sudo().create(values)
            vals['name'] = self.env['ir.sequence'].next_by_code('oeh.medical.patient.ambulatory')
            medical_patient_ambulatory = models.Model.create(self, vals)
            return medical_patient_ambulatory
        else:
