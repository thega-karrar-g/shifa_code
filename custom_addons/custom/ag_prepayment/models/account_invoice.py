...
class AccountInvoice(models.Model):
    ...
    #
    #             if mv_line.is_landed_costs_line == True:
    #                 landed_cost += mv_line.price_subtotal
    #             else:
    #                 to_qty += mv_line.quantity
    #                 to_cost +=  mv_line.price_subtotal
    #         for mv_line in inv.invoice_line_ids:
    #             if to_cost:
    #                 per_unit = (landed_cost / to_cost)
    #                 value = (mv_line.price_subtotal*(1/inv.currency_rate)) * per_unit
    #             else:
    #                 value = 0.0
    #             mv_line.with_context(context).asset_create(value)
    #     return result
...
class AccountInvoiceLine(models.Model):
    ...
    def asset_create(self):#landed_cost
        if self.asset_category_id:

            if self.name:
                name = self.name
            else:
                name = "[%s]%s"%(self.product_id.default_code,self.product_id.name)
