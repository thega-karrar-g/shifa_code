odoo.define('smartmind_dashboard.DashboardData', function (require) {
    ...
    var OehDashBoard = AbstractAction.extend({
        ...
        events: {
            ...
            // start custom
            'click .load_car_cont_con_active': 'load_car_cont_con_active',
            'click .load_car_cont_con_un_active': 'load_car_cont_con_un_active',
            // end custom
        ...
        // start custom
         async get_contracts () {
            ...
            try {
                ...
                const def0 = await self.searchModel.env.services.rpc({
                    model: 'sm.caregiver.contracts',
                    method: 'search_read',
                    args: [[['active', '=', true], ['state', 'in', ['active']]], ['caregiver']],
                //     args: [[
                //     ['active', '=', true],
                //     ['state', 'in', ['active', 'renew', 'assign_caregiver']]
                // ], ['caregiver']],
                });

                const values = def0
                    .map(obj => obj.caregiver && obj.caregiver.length ? obj.caregiver[0] : null)
                    .filter(id => id !== null); // Remove nulls just in case

                console.log('Caregiver IDs:', values);
           ...
           load_car_cont_con_active: async function (e) {
            e.stopPropagation();
            e.preventDefault();

            // Fetch caregiver IDs from contracts
            await this.get_contracts();

            console.log('Contracts list:', this.contracts);

            // Ensure we only proceed if there are valid IDs
            if (this.contracts.length === 0) {
                return this.do_notify('No caregivers found in active contracts', '');
            }

            // Open action with domain filter on caregivers
            this.do_action({
                name: _t("Caregiver Contract"),
                type: 'ir.actions.act_window',
                res_model: 'oeh.medical.physician',
                view_mode: 'tree,form,kanban',
                views: [[false, 'list'], [false, 'form'], [false, 'kanban']],
                domain: [['id', 'in', this.contracts]],
                target: 'current',
            }, {
                reverse_breadcrumb: this.reverse_breadcrumb
            });
        },
        load_car_cont_con_un_active: async function (e) {
            e.stopPropagation();
            e.preventDefault();

            // Open action with domain filter on caregivers
            this.do_action({
                name: _t("Stay-in Cargiver"),
                type: 'ir.actions.act_window',
                res_model: 'oeh.medical.physician',
                view_mode: 'tree,form,kanban',
                views: [[false, 'list'], [false, 'form'], [false, 'kanban']],
                //domain: [['id', 'not in', this.contracts],['role_type','==','C']],
                domain: [['role_type', '=', 'C'], ['id', 'not in', this.contracts]],
                //domain: [['active_contract_count', '>', 0],['role_type','=','C']],
                target: 'current',
            }, {
                reverse_breadcrumb: this.reverse_breadcrumb
            });
