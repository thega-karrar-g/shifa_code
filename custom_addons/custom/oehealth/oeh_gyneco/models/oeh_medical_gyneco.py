...
class OeHealthGyneco(models.Model):
    ...
    @api.model
    def create(self, vals):
        company = self.env['res.company']._company_default_get('oeh.medical.gyneco')
        search_sequence = self.env['ir.sequence'].search(
            [('code', '=', 'oeh.medical.gyneco'), ('company_id', 'in', [company.id, False])], order='company_id')
        if not search_sequence:
            values = {
                'name': 'Gynaco (' + str(company.name) + ')',
                'code': 'oeh.medical.gyneco',
                'company_id': company.id,
                'prefix': 'GY',
                'padding': 5,
            }
            self.env['ir.sequence'].sudo().create(values)
            vals['name'] = self.env['ir.sequence'].next_by_code('oeh.medical.gyneco')
