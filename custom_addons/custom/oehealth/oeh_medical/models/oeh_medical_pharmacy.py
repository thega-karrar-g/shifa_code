...
class OeHealthPharmacyLines(models.Model):
    ...
    doctor = fields.Many2one('oeh.medical.physician', string='Doctor', help="Current primary care / family doctor", domain=[('is_pharmacist','=',False)], readonly=True, states={'Draft': [('readonly', False)]})
    ...
    def _onchange_prescription_id_values(self,prescription_id):
        ...
        if vals:
            for va in vals:
                ...
                prescription_ids.append((0, 0, pres))
                # prescription_ids += [pres]
            # res['value'].update({
            #     'prescription_lines': prescription_ids,
            # })
            res['value']['prescription_lines'] = prescription_ids
            _logger.info('\n\n>>>>>>>>>>>>>>>>>>22222222>>>\n\n %s ------ %s' % (res,prescription_ids))
        return res
    ...
    def action_prescription_invoice_create(self):
        ...
        inv_ids = []
        default_journal = self._default_account()
