...
class SmCaregiverContracts(models.Model):
    ...
    def _calculate_vat(self):
        for rec in self:
            if rec.ssn and rec.ssn[0] == '2':
                rec.vat = (rec.service_price * int(rec.no_caregiver) - rec.discount_val) * 0.15
            else:
                rec.vat = 0.0
    ...
    def _calculate_discount(self):
        for rec in self:
            if rec.discount_id:
                rec.discount_val = rec.service_price * (rec.discount_id.fixed_type / 100) * int(rec.no_caregiver)
            else:
                rec.discount_val = rec.service_price * (rec.discount / 100) * int(rec.no_caregiver)
        ...
        if records:
            ...
            for record in records:
                ...
                if record.patient_id.mobile:
                    msg = "مرحبا " + record.patient_requested_id.name + ", هذا تذكير ودي بأن عقدك من المقرر أن ينتهي في غضون 5 أيام. لمواصلة الاستمتاع بخدماتنا، يرجى تجديد عقدك قريبًا. إذا كنت ترغب في إنهاء عقدك، فيمكنك القيام بذلك بسهولة من خلال تطبيقنا المحمول. نشكرك على اختيارنا"
    ...
    def create(self, vals):
        ...
        if 'patient_id' not in vals and 'patient_requested_id' in vals and vals['patient_requested_id']:
            vals['patient_id'] = vals['patient_requested_id']
