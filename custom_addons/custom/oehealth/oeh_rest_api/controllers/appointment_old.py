...
class SmartMindAppointmentRESTAPIController(http.Controller, SmartMindSharedMethods):
    ...
    def create_appointment(self, **post):
        ...
        if model:
            try:
                ...
                patient = False
                count_patient = request.env['oeh.medical.patient'].sudo().search_count([('id', '=', int(patient_id))])
                if count_patient > 0:
                ...
                if 'appointment_date' in post:
                    appointment_date = datetime.strptime(post.get('appointment_date'), '%Y-%m-%d %H:%M:%S')
                else:
                    appointment_date = datetime.now()

                appointment_end = self._get_tele_hvd_appointment_end(appointment_date)

                appointment_date = datetime.strptime(str(appointment_date).replace('+00:00', ''), date_format)

                if appointment_date > datetime.now():
                    count_booking_time = request.env[model_name].sudo().search_count(
                        [('appointment_date', '>=', convert_date_to_utc(str(appointment_date))),
                         ('appointment_end', '<=', convert_date_to_utc(str(appointment_end)))])

                    if count_booking_time > 0:
                        return invalid_response('appointment already exists',
                                                'Please select another time, the selected time has been already taken!')
                    else:
                        # get files name
                        ...
                        # ----------------------------------------------------------------------------------------------
                        values['appointment_date_only'] = appointment_date.strftime('%Y-%m-%d')
                        values['appointment_time'] = self._convert_to_time(appointment_date.strftime('%H:%M'))
                        values['appointment_date'] = appointment_date
                        values['appointment_end'] = datetime.strptime(str(convert_date_to_utc(str(appointment_end))).replace('+00:00', ''), date_format)
                        # ----------------------------------------------------------------------------------------------
                        jitsi_link = False
                        if payment_type == 'pay':
                    ...
                    data = {
                        ...
                        'count_booking_time': str(count_booking_time),
                        'appointment_date': str(appointment_date),
                        'appointment_end': str(appointment_end),
                    }
                    return valid_response(data)
                else:
                    return invalid_response('cannot_book_before_today',
                                            'Sorry, you can only book an appointment start from tomorrow date!')
            ...
            try:
                ...
                if appointment_date > datetime.now():
                    ...
                    if count_booking_time > 0:
                        ...
                        values['appointment_date'] = str(appointment_date).replace('+00:00', '')

                        # ----------------------------------------------------------------------------------------------
                        values['appointment_date_only'] = appointment_date.strftime('%Y-%m-%d')
                        values['appointment_time'] = self._convert_to_time(appointment_date.strftime('%H:%M'))
                        values['appointment_date'] = datetime.strptime(
                            str(convert_date_to_utc(str(appointment_date))).replace('+00:00', ''), date_format)
                        values['appointment_end'] = datetime.strptime(
                            str(convert_date_to_utc(str(appointment_end))).replace('+00:00', ''), date_format)
                        # ----------------------------------------------------------------------------------------------
                        if payment_type == 'pay':
                            values['payment_type'] = 'Payment Process'
                        else:
    ...
    def request_cancellation_confirmed_appointment(self, **post):
        ...
        try:
            ...
            if apt_type == 'tele':
                ...
                domain_state = [('state', 'in', ['scheduled', 'head_doctor', 'head_nurse', 'operation_manager', 'team'])]
            elif apt_type == 'phy':
            ...
            if count_apt_patient > 0:
                ...
                if can_cancel_count:
                    ...
                    return invalid_response('not_allowed', 'Sorry! you cannot cancel this appointment, its in progress or in start state')
                # appointment = request.env[model_name].sudo().search(domain, limit=1)
            else:
                return invalid_response('invalid data',  'Sorry! either this appointment id is not belong to this patient id or there is no appointment for this patient id')
        except Exception as e:
    ...
    def get_appointment(self, user_type=None, user_id=None, **payload):
        ...
        model = request.env[self._model].sudo().search([('model', '=', model_name)], limit=1)
        payload[
            'fields'] = "['id', 'name', 'patient', 'doctor', 'appointment_date', 'appointment_time', 'institution', 'urgency_level', 'jitsi_link', 'comments', 'patient_status', 'state']"
    ...
    def get_hvd_appointment(self, user_type=None, user_id=None, **payload):
        ...
        model = request.env[self._model].sudo().search([('model', '=', model_name)], limit=1)
        payload[
            'fields'] = "['id', 'name', 'patient', 'doctor', 'appointment_date', 'appointment_time', 'institution', 'urgency_level', 'comments', 'patient_status', 'state']"
    ...
    def get_appointment_count_all(self, patient_id=None):
        ...
        ol_appointment = request.env['oeh.medical.appointment'].sudo().search_count(
        ...
        data = {
            'tele_appointment': ol_appointment,
    ...
    def set_payment_status(self, **post):
        ...
        try:
            ...
            if ref > 0:
                ...
                self.create_requested_payment(patient_id, datetime.now().date(), deduction_amount, payment_reference, details, appointment_type, apt_date)
        ...
        try:
            ...
            if count_payment > 0:
                ...
                if payment_reference != '':
                    action = self._update_payment_reference(payment_id, 'Paid', payment_reference, float(deduction_amount))
                    if action:
                    ...
                    action = self._update_payment_reference(payment_id, 'Reject', payment_reference, float(deduction_amount))
                    if action:
    ...
    def get_appointments_discount(self, **payload):
        ...
        model = request.env[self._model].sudo().search([('model', '=', model_name)], limit=1)
        payload['fields'] = "['id', 'name', 'start_date', 'exp_date', 'state', 'discounts_type', 'fixed_type', 'apply_to', 'hhc', 'tele', 'pcr', 'hvd', 'physiotherapy', 'customer_code']"
