...
class SmCaregiverContracts(models.Model):
    ...
    def _calculate_vat(self):
        for rec in self:
            ...
            if rec.ssn and rec.ssn[0] == '2':
                vat = (rec.service_id.list_price * int(rec.no_caregiver) - rec.discount_val) * 0.15
            
            additional_vat = (rec.additional_service_id.lst_price * int(rec.no_caregiver)) * 0.15
            vat = vat + additional_vat
    ...
    service_id = fields.Many2one('sm.shifa.service', string='Service', required=True,
                                 ...
                                 states={'draft': [('readonly', False)],'renew': [('readonly', False)], 'unpaid': [('readonly', False)]}, tracking=True)
    additional_service_id = fields.Many2one('sm.shifa.service', string='Transporting Fee',
                                            domain=[('service_type', '=', 'Car')], readonly=True,
                                            states={'draft': [('readonly', False)]}, tracking=True)
    additional_service_id = fields.Many2one('product.product', string='Transporting Fee',
    ...
    discount_id = fields.Many2one('sm.shifa.discounts', string='Discount Name',
                                  domain=[('state', '=', 'Active'),('caregiver','=',True)], readonly=True,
    ...
    patient_diseased = fields.Boolean(copy=False,string='Patient Deceased')
    ...
    def _compute_remaining_days(self):
        for rec in self:
            ...
            if rec.termination_date and rec.ending_date:
    ...
    def get_credit_note_lines(self):
        ...
        if self.state in ['cancel', 'evaluation', 'assign_caregiver']:
            ...
            if self.service_id.refund_percent > 0 and not self.patient_diseased:
    ...
    def create_journal_entries(self):
        # Fetch account and journal IDs from config parameters
        income_account_id = int(self.env['ir.config_parameter'].sudo().get_param('sm_caregiver.caregiver_deferred_income_account_id'))
        credit_account_id = int(self.env['ir.config_parameter'].sudo().get_param('sm_caregiver.caregiver_credit_account_id'))
        journal_id = int(self.env['ir.config_parameter'].sudo().get_param('sm_caregiver.caregiver_journal_id'))

        if not all([income_account_id, credit_account_id, journal_id]):
            raise UserError("Missing configuration: Ensure that all caregiver accounts and journal IDs are set.")

        # Calculate how many journal entries to create
        nb_entries = int(self.duration / 10)
        for i in range(nb_entries):
            entry_date = self.starting_date + timedelta(days=30 * i)
            ...
            self.env['account.move.line'].sudo().create([
                {
                    ...
                    'credit': self.service_id.list_price / nb_entries,
                    'debit': 0.0,
                    'partner_id': self.patient_requested_id.partner_id.id,
                    'name': self.name,
                },
                {
                    ...
                    'debit': self.service_id.list_price / nb_entries,
                    'credit': 0.0,
                    'partner_id': self.patient_requested_id.partner_id.id,
                    'name': self.name,
                }
    ...
    def set_to_cancelled(self):
        if self.state == 'unpaid':
            ...
            ctx = {
                'form_view_ref': 'sm_caregiver.sm_caregiver_patient_diseased',
            }
            return {
                'type': 'ir.actions.act_window',
                'res_model': 'sm.caregiver.contracts',
    ...
    def set_to_unpaid(self):
        return self.write({'state': 'holdreq'})

    def check_completed_contracts(self):
        date = (fields.Datetime.now() + timedelta(hours=3)).date()
        records = self.sudo().search([('ending_date', '!=', False), ('ending_date', '<', date),
                                      ('state', '=', 'active')])
    ...
    def check_unpaid_contracts(self):
        date_threshold = (fields.Datetime.now() + timedelta(days=2, hours=12)).date()
        records = self.sudo().search([('state', '=', 'unpaid'), ('date', '<=', date_threshold)])
        for rec in records:
            ...
            for record in records:
                ...
                if record.patient_id.mobile:
                    msg = (
                        ...
                        "لمواصلة الاستمتاع بخدماتنا. إذا كنت ترغب في إنهاء عقدك، فيمكنك القيام بذلك بسهولة من خلال تطبيقنا المحمول. "
    ...
    def write(self, vals):
        ...
        if 'state' in vals and vals['state'] in ['evaluation','paid'] and not self.link:
    ...
    def check_paid(self):
        for request_payment_id in self.request_payment_ids.filtered(lambda l: l.state not in ['Paid', 'Done','renew']):
            if request_payment_id and not self.pro_pending and self.payment_made_through == 'call_center':
    ...
    def action_add_link(self):
        report = self.env.ref('sm_caregiver.caregiver_contract_report', False)
        for rep in self:
            if report:
                pdf_content, _ = report._render_qweb_pdf([rep.id])
                pdf_content = base64.b64encode(pdf_content)
                attachment = self.env['ir.attachment'].sudo().create({
                    'name': self.name + '.pdf',
                    'res_model': 'sm.caregiver.contract',
                    'datas': pdf_content,
                    'res_id': self.id
                })
                self.attachment_ids = [(4, attachment.id)]
                self.link = self._get_pdf_link(attachment)


    def _get_pdf_link(self, attachment):
        link = ""
        config_obj = self.env['ir.config_parameter'].get_param('web.base.url')
        attachment_url = config_obj + "/web/attachments/token/"
        link = attachment_url + str(attachment.access_token)
        return link
    ...
    def set_to_active(self):
        date = self.date or (fields.Datetime.now() + timedelta(hours=3)).date()
        if self.state == 'renew':
            date = self.create_date.date() if not self.starting_date else self.starting_date
        self.starting_date = date
        ctx = {
    ...
    def terminate(self):
        ...
        posted_moves = self.env['account.move'].sudo().search([
            ('journal_id','=', journal_id),
            ('caregiver_contract_id','=',self.id),
            ('state','=','posted')
        ])
        ...
        for move in posted_moves:
            reversal = self.env['account.move.reversal'].with_context(active_ids=move.ids, active_model='account.move').sudo().create({
                'date_mode': 'entry',
                'journal_id': journal_id,
            })
            
            reversal.reverse_moves()
            for move in reversal.new_move_ids:
                move.sudo().write({"patient": self.patient_requested_id.id, "caregiver_contract_id": self.id})
...
class Users(models.Model):
    ...
    company_ids = fields.Many2many('res.company',default=_default_companies)
